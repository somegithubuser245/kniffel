...
public class MainScreen extends JFrame {
	...
	public void roll(boolean debug) {
		...
		// gehaltene Würfel and Wuefel-Klasse übergeben damit diese nicht geändert
		// werden
		if (rollCounter > 0 && !debug)
			transmitSavedDice();

		// gamecontroller aktiviert würfel funktion der würf.Klasse und übergibt GUI das
		// würfelergebnis
		...
		if (rollCounter == 3)
			rollButton.setEnabled(false);
		// entfernt comboToShow JPanel und aktualisiert dann die ScoreTable und fügt
		// dann die comboToShow wieder ein
		refreshCombosToShow();
	}
	...
	private void addTableUI() {
		...
		confirm.addActionListener(e -> {
			System.out.println(
					"Auswahl Bestätigt: " + getChoosenComboIndex() + " mit " + getChoosenPoints() + " Punkten.");

			if (choosenPoints != 0 && !zeroPointsWarning) {
				System.out
						.println("Gespeichert: Kombi: " + getChoosenComboIndex() + " mit Wert: " + getChoosenPoints());

				nextPlayer.setEnabled(true);
			} else if (zeroPointsWarning) {
				confirm.setText("Combo gestrichen :(");
				nextPlayer.setEnabled(true);
		...
		nextPlayer.addActionListener(e -> {
			// übergibt combo an gamecontroller als index in combo array der gamecontroller
			// klasse
			...
			if (GameController.getGameOver()) {
				...
				// nextplayer im gamecontroller ausführen und mit return wert currentplayer
				// aktualisieren
	...
	private void fillScoreTable(String[] firstColumn, JPanel table, GridBagConstraints c, Dimension sizeFeld) {
		...
		for (int column = 1; column < numberOfPlayers + 1; column++) {
			...
			for (int row = 1; row < 20; row++) {
				...
				if (pointsReal[column - 1][row - 1] < 0 && (column - 1) == currentPlayer
						&& pointsToShow[column - 1][row - 1] >= 0) {
					...
					newPoints.addActionListener(e -> {
						...
						// aktualisiert die SpielerAuswahl Variablen mit den für den Knopf
	...
	private void openGameOverDialog() {
		...
		winner.setText("Der Gewinner ist: " + GameController.getWinnerName() + " mit "
				+ GameController.getWinnerPoints() + " Punkten :)");
